# COPY — Apneia na Prática

> Regra: usar esta copy como fonte textual da landing page. Não reescrever, resumir ou inventar novas promessas sem aprovação.

## DOBRA 1 — HERO (v1.3.0)

> Fonte: HeroLP-testing, fornecido pelo usuário para integração em 18/09/2026.
> Esta seção substitui a Hero antiga; os textos abaixo foram importados sem criação de novas alegações pelo implementador.

# Aprenda a tratar Apneia do sono no seu consultório

Saiba o que observar, quais perguntas fazer e quando o sono merece uma investigação mais aprofundada.

**Treinamento para dentistas por R$ 47,00, com cartilha prática para o uso no consultório.**

**CTA:** QUERO ACESSAR AGORA

**Microcopy:** Pagamento 100% seguro processado via Hotmart. Seus dados estão protegidos.

---

## DOBRA 2 — IDENTIFICAÇÃO

**Eyebrow:** ISSO ACONTECE NA SUA CONSULTA?

# Nem todo paciente chega dizendo que tem um problema de sono.

Muitas vezes, os sinais aparecem de forma fragmentada durante a consulta. O desafio não é apenas perceber que algo pode estar acontecendo — é saber quais informações merecem atenção e como conectá-las.

### Card 1 — Ronco pouco percebido ou negado
O paciente pode dizer que não ronca ou simplesmente não saber com que frequência isso acontece.

### Card 2 — Sono não reparador
Ele dorme, mas continua acordando cansado ou com a sensação de que não descansou.

### Card 3 — Cansaço e sonolência durante o dia
Sintomas que podem parecer parte da rotina, mas que ganham outro significado quando analisados dentro do contexto do paciente.

### Card 4 — Informações que aparecem desconectadas
O desafio muitas vezes não está em um único sinal, mas em saber como juntar anamnese, sintomas, observações e ferramentas de apoio.

**CTA:** QUERO APRENDER A INVESTIGAR MELHOR

---

## DOBRA 3 — MECANISMO CLÍNICO

# Reconhecer um sinal é apenas o começo.

O que muda a investigação é saber o que perguntar, o que observar, como organizar as informações e quando decidir o próximo passo.

### 01 — PERGUNTAR
Conduzir uma anamnese mais direcionada sobre sono e sintomas relacionados.

### 02 — OBSERVAR
Identificar sinais relevantes dentro do contexto clínico do paciente.

### 03 — ORGANIZAR
Conectar informações da anamnese, observação clínica e ferramentas de apoio.

### 04 — DECIDIR
Entender quando existem motivos para aprofundar a investigação ou encaminhar o paciente.

**Nenhum sinal isolado equivale a um diagnóstico.**

**CTA:** QUERO APRENDER ESSE PROCESSO

---

## DOBRA 4 — PRODUTO E OFERTA

**Eyebrow:** O QUE VOCÊ RECEBE

# Apneia na Prática: um treinamento direto ao ponto para levar mais critério à investigação durante a consulta.

Você vai aprender a reconhecer sinais de possível apneia, conduzir perguntas mais direcionadas e organizar melhor as informações antes de decidir quando a investigação merece avançar.

### No treinamento, você vai aprender:

- Como trazer o sono para a anamnese de forma mais direcionada.
- O que observar durante a consulta.
- Como analisar sinais dentro do contexto do paciente.
- Como utilizar questionários como apoio à investigação.
- Como evitar conclusões a partir de sinais isolados.
- Quando existem motivos para aprofundar ou encaminhar.

### Você também recebe

**Uma cartilha prática para uso no consultório**, pensada para ajudar a levar o conteúdo do treinamento para a rotina clínica.

**APNEIA NA PRÁTICA**  
Treinamento + cartilha prática

# R$ 47,00

**CTA:** QUERO ACESSAR AGORA

Pagamento seguro via Hotmart.

---

## DOBRA 5 — PROFESSOR + GARANTIA + SUPORTE

**Eyebrow:** QUEM CONDUZ O TREINAMENTO

# Prof. Dr. Eduardo Rollo

Eduardo Rollo conduz o treinamento com foco na investigação de possíveis sinais de apneia aplicada ao contexto clínico.

Ao longo da aula, você entende como perguntas de anamnese, observação clínica, ferramentas de apoio e encaminhamento podem ser organizados dentro de um raciocínio mais estruturado para a rotina do consultório.

### 7 dias de garantia

Você tem **7 dias para acessar o treinamento e avaliar o conteúdo**. Se decidir que ele não é para você, poderá solicitar o reembolso dentro do prazo da garantia.

### Pagamento seguro

Sua compra é processada pela Hotmart em ambiente protegido.

### Precisa de ajuda?

Ficou com alguma dúvida sobre pagamento ou acesso ao treinamento? Entre em contato com nossa equipe de suporte.

# Comece a reconhecer possíveis sinais de apneia com mais critério na sua consulta.

**Apneia na Prática**  
Treinamento + cartilha prática para uso no consultório.

# R$ 47,00

**CTA FINAL:** QUERO ACESSAR AGORA

---

## RODAPÉ

Este é um conteúdo educacional destinado a dentistas. Sinais, sintomas, questionários e observações clínicas não equivalem, isoladamente, a diagnóstico de apneia do sono. O treinamento não substitui avaliação clínica individual nem exames diagnósticos quando indicados.

**© 2026 Apneia na Prática. Todos os direitos reservados.**
